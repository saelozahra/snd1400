...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("SingleCandidateLayout")
	...
	ToolBar.Padding=Array As Int(0,SaeloZahra.StatusBarHeight,0,0dip)
	ToolBar.Height=SaeloZahra.MaterialActionBarHeight+SaeloZahra.StatusBarHeight
...
'	CandidateJob.Download(SaeloZahra.JsonUrl&"product/index/"&CandidateID)
	...
	CategoryLbl.Typeface = SaeloZahra.fontBold : CategoryLbl.Color=SaeloZahra.Color
	TagLbl.SetLayout((CategoryLbl.Left+CategoryLbl.Width+14dip),TagLbl.top,TagLbl.Width,TagLbl.Height) : TagLbl.Color=SaeloZahra.Color : TagLbl.Typeface=SaeloZahra.Font
...
Sub SimulateAddItem
	...
	ImageJob.Download("https://saelozahra.ir/wp-content/uploads/2021/01/4_5866090371808756967-scaled.jpg")
	ImagesSliderPanel.SetVisibleAnimated(313,True)

	Dim SliderHTML As String = File.ReadString(File.DirAssets,"slider_1.html")
	SliderHTML = SliderHTML & " <div class='item'><img src='https://defapress.ir/files/fa/news/1395/9/11/51398_996.jpg'></div>"
	SliderHTML = SliderHTML & " <div class='item'><img src='https://saelozahra.ir/wp-content/uploads/2020/12/photo_%DB%B2%DB%B0%DB%B2%DB%B0-%DB%B1%DB%B1-%DB%B1%DB%B4_%DB%B0%DB%B3-%DB%B3%DB%B9-%DB%B5%DB%B5-1-e1607809180436.jpg'></div>"
	SliderHTML = SliderHTML & " <div class='item'><img src='https://saelozahra.ir/wp-content/uploads/2021/01/4_5866090371808756967-scaled.jpg'></div>"
	SliderHTML = SliderHTML & File.ReadString(File.DirAssets,"slider_2.html") & "<style>#owl-demo .item img{width: 100%; height: 100%; top: 0; left: 0; max-width:none; max-height:none; object-fit:cover; } .owl-buttons {top: 35%;} </style>"
	SliderWV.LoadHtml(SliderHTML)
	
	CF.Title=SaeloZahra.CSBTitle(" محمدامیر لطفی")
	tel 		= "09206208105"
	Dim cat_title As String = "کمیسیون فرهنگی"
	CategoryLbl.Text = cat_title
	Dim viewcount As String = 110+Rnd(88,313)
	Dim CSBText As CSBuilder
	CSBText.Initialize.Append("دکتر محمد امیر لطفی 30 سال پیش در چنین روزی در خانواده ای مذهبی و در شهر شیراز چشم به جهان گشود "&CRLF&"دیری نپائید که دکتر لطفی علاقه مند به مباحث الکترونیک و کامپیوتر شد و به فراگیری این علوم در دارالمکتب باهنر نمود"&CRLF&"هم اکنون او راننده اسنپ اسنپ است و در تپ‌سی امرار معاش می‌کند...").Append(CRLF).Size(10).Color(Colors.DarkGray).Append("افراد آنلاین در این صفحه: ").Bold.Append(viewcount).Pop.Append(" نفر ").PopAll
	TextLbl.Text= CSBText
	Dim city As String = "شیراز"
	TagLbl.Text = city
	
	PropertyNumber = 0
	P1Lbl.Text=BuildPropertyItemStyle("نام","محمد امیر لطفی")
	P1Lbl.Width=-1
	P2Lbl.Text=BuildPropertyItemStyle("تحصیلات","لیسانس الهیات")
	P2Lbl.Width=-1
	P3Lbl.Text=BuildPropertyItemStyle("حوزه تخصصی","کمیسیون فرهنگی")
	P3Lbl.Width=-1
	P4Lbl.Text=BuildPropertyItemStyle("شهر","شیراز")
	P4Lbl.Width=-1
	P5Lbl.Text=BuildPropertyItemStyle("موافق با شفافیت اموال مسئولین","بله")
	P5Lbl.Width=-1
	P6Lbl.Text=BuildPropertyItemStyle("سابقه مسئولیت","مسئول گروه سرود مدرسه")
	P6Lbl.Width=-1
	Responsive
...
Sub Responsive
	...
	TextLbl.Height = SU.MeasureMultilineTextHeight(TextLbl,TextLbl.text)+7%y
	TextPanel.Height = TextLbl.Height
	TagLbl.Top = TextLbl.Height-8%y
	CategoryLbl.Top = TextLbl.Height-8%y
